# Evidence and Report Index

## Purpose

This index maps major claims in this documentation set to repository evidence artifacts.

## Core Tracking Artifacts

- `CONTEXT.md`
- `PROGRESS.MD`
- `CHANGELOG.md`
- `RELEASE_NOTES/`

## Research Artifacts

- `research/From Apps to Answers - Connecting Public Sector Data to AI with MCP/`
- `research/Deep Research Report/Research Apps to Answers_ Connecting Public Sector Data to AI with MCP.md`
- `research/ons_dataset_selection/`
- `research/map_delivery_research_2026-02/`

## Troubleshooting Artifacts

- `docs/troubleshooting.md`
- `troubleshooting/Forensic and Methodological Deep Research Study for an MCP Geospatial Server.md`
- `troubleshooting/claude-system-instructions-hide-mcp.md`
- `troubleshooting/nomis-and-toolsearch-deep-analysis-2026-03-03.md`
- `troubleshooting/validating-nomis-trace-evidence-2026-03-03.md`
- `troubleshooting/peat-survey-failure-deep-analysis-2026-03-03.md`
- `troubleshooting/peat-survey-trace-evidence-2026-03-03.md`

## Evaluation and Metrics Artifacts

- `tests/evaluation/questions.py`
- `tests/evaluation/harness.py`
- `tests/evaluation/rubric.py`
- `tests/evaluation/evaluation_results.json`
- `docs/evaluation.md`
- `docs/reports/mcp_geo_codex_long_horizon_summary_2026-03-04.*`
- `docs/reports/repo_extent_complexity_2026-02-25.*`
- `docs/reports/public_release_security_review_2026-03-04.md`

## Design and UI Interop Artifacts

- `docs/reports/mcp_apps_window_constraints_review_2026-03-01.md`
- `docs/design/figma/mcp_figma_setup_and_capture_runbook.md`
- `playground/tests/compact_windows/`

## Support Matrix and Specifications

- `docs/spec_package/`
- `docs/map_delivery_support_matrix.md`
- `docs/spec_tracking.md`

## External Primary Sources Used in This Documentation Set

- <https://modelcontextprotocol.io/specification/2025-11-25/changelog>
- <https://modelcontextprotocol.io/specification/2025-03-26/changelog>
- <https://modelcontextprotocol.io/specification/2025-06-18>
- <https://modelcontextprotocol.io/blog/mcp-updates-from-july-2025-to-january-2026>
- <https://www.anthropic.com/news/claude-code>
- <https://www.anthropic.com/news/claude-code-best-practices>
- <https://code.visualstudio.com/updates/v1_99>
- <https://openai.com/index/introducing-codex/>
- <https://openai.com/index/introducing-the-codex-app/>
