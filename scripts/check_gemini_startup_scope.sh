...
WRAPPER="${MCP_GEMINI_WRAPPER:-$REPO_ROOT/scripts/gemini-mcp-local}"
BUILD_MODE="${MCP_GEO_DOCKER_BUILD:-never}"
TIMEOUT_SEC="${MCP_GEMINI_SCOPE_TIMEOUT_SEC:-45}"
...
def run_count(toolset: str | None, include: str | None) -> tuple[int, list[str]]:
    ...
    if not line:
        raise RuntimeError(json.dumps({"error": "empty_stdout", "stderrTail": stderr_tail}))
    try:
