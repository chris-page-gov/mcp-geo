...
def test_get_ons_observations_basic():
    resp = client.get('/resources/read?name=ons_observations&limit=2&page=1')
    assert resp.status_code == 200
...
def test_get_ons_observations_second_page_and_etag_cache():
    ...
    first = client.get('/resources/read?name=ons_observations&limit=2&page=2')
    assert first.status_code == 200
    etag = first.headers.get('ETag')
    assert etag
    cached = client.get('/resources/read?name=ons_observations&limit=2&page=2', headers={'If-None-Match': etag})
    assert cached.status_code == 304


def test_get_ons_observations_final_page_no_next():
    # There are 5 observations; with limit 2 page 3 is final (obs 5)
    final = client.get('/resources/read?name=ons_observations&limit=2&page=3')
    assert final.status_code == 200
    body = resource_json(final)
    assert body['data']['nextPageToken'] is None


def test_get_ons_observations_filter_geography():
    resp = client.get('/resources/read?name=ons_observations&geography=K02000001')
    assert resp.status_code == 200
    body = resource_json(resp)
...
def test_get_ons_observations_filter_measure():
    resp = client.get('/resources/read?name=ons_observations&measure=chained_volume_measure')
    assert resp.status_code == 200
    body = resource_json(resp)
