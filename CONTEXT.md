...
- Maintaining the new 2026-04-04 LandIS soil-screening MVP under
  ...
  product registry plus LandIS-backed MCP tools for product listing, metadata,
  Soilscapes point lookup, Soilscapes area summaries, and an area-based pipe
  corrosion / shrink-swell screening surface. Runtime behavior is explicitly
  split between offline-safe registry/resources and warehouse-backed geospatial
  queries, with `LANDIS_ENABLED` / `LANDIS_LIVE_ENABLED` controlling the live
  path and the PostGIS warehouse contract carrying provenance/version data for
  every dataset used in derived outputs. The current evaluation harness keeps
  these LandIS tools on the specialist-surface path until the canonical
  question bank is expanded to exercise them directly. The same 2026-04-04
  workstream now also includes an authenticated Atlas-based inventory of the
  protected LandIS portal, generated by `scripts/landis_portal_inventory.py`
  and published at
  `research/landis-data-source/landis_portal_inventory_2026-04-04.json` plus
  `docs/reports/landis_portal_inventory_2026-04-04.md`, so the protected
  ArcGIS catalog, docs/pages, and supporting app surfaces are indexed without
  storing session credentials. The follow-on `scripts/landis_portal_download.py`
  now uses the same Atlas-authenticated route to mirror the indexed portal
  items to local storage, capturing raw item payloads and chunked Feature
  Service exports under an operator-chosen destination such as the external
  SSD staging area. That authenticated mirror is now complete (`106` items,
  `33` data services, `73` non-feature payloads, `0` manifest errors) and the
  next priority is the committed phase-2 surfacing review in
  `docs/reports/landis_phase_2_surfacing_plan_2026-04-04.md`: keep the MVP
